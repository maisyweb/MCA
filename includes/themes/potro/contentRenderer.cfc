...
<cfcomponent extends="mura.cfobject">
	...
	<cffunction name="addlink" output="false" returntype="string">
		...
		<cfset link='<a href="#href#"#iif(len(arguments.target) and arguments.target neq '_self',de(' target="#arguments.target#"'),de(""))##iif(len(theClass),de(' class="#theClass#"'),de(""))##iif(len(arguments.id),de(' id="#arguments.id#"'),de(""))##iif(arguments.showCurrent,de(' #replace(arguments.aCurrentAttributes,"##","####","all")#'),de(""))##iif(arguments.isParent and len(arguments.aHasKidsAttributes),de(' #replace(arguments.aHasKidsAttributes,"##","####","all")#'),de(""))#>#HTMLEditFormat(arguments.title)#' />
		<cfif arguments.isParent AND listLen(arguments.filename, '/') EQ 1>
			<cfset link = link & '<i class="icon-angle-down"></i>'>
		</cfif>
		<cfset link = link & '</a>'>
		<cfreturn link>
	</cffunction>
			...
			<cfsavecontent variable="theNav">
				<cfoutput><ul<cfif len(arguments.id)> id="#arguments.id#"</cfif><cfif len(arguments.class)> class="#arguments.class#"</cfif>>
					<cfloop from="#theOffset#" to="1" index="I" step="-1">
						<cfif i neq theOffset>
							<cfset activeClass = "active">
						<cfelse>
							<cfset activeClass = "">
						</cfif>
						<cfif I neq 1><li class="#activeClass# #iif(I eq theOffset,de('first'),de(''))#"><cfif i neq theOffset>#arguments.separator#</cfif>#addlink(this.crumbdata[I].type,this.crumbdata[I].filename,this.crumbdata[I].menutitle,'_self','',this.crumbdata[I].contentid,this.crumbdata[I].siteid,'',application.configBean.getContext(),application.configBean.getStub(),application.configBean.getIndexFile(),variables.event.getValue('showMeta'),0)#</li><cfelse><li class="#activeClass# #iif(arraylen(this.crumbdata),de('last'),de('first'))#">#arguments.separator##addlink(this.crumbdata[1].type,this.crumbdata[1].filename,this.crumbdata[1].menutitle,'_self','',this.crumbdata[1].contentid,this.crumbdata[1].siteid,'',application.configBean.getContext(),application.configBean.getStub(),application.configBean.getIndexFile(),variables.event.getValue('showMeta'),0)#</li></cfif>
					</cfloop>
				</ul></cfoutput>
			</cfsavecontent>
		</cfif>
